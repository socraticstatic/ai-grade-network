...
function addendumVals(c, s, set, est, ob, inv, go, findingCard, totalSave, est0) {
  ...
  const startOrder = () => { c.setState({ screen: 's4', ...newOrder({ ...prefillCompose(est), path: pathPick, buy: buyPick }) }); syncHash('s4', s.layer, s.tab); };
