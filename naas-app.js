...
export function vals(c) {
  ...
  const bulkAttach = () => {
    ...
    c.setState({ screen: 's4', compose: { ...prefillCompose(est), bulk: what, qty: cnt }, parsedNote: `Attach ${what}. One order, one policy, ${cnt.toLocaleString('en-US')} circuits.` });
    syncHash('s4', s.layer, s.tab);
  };
  ...
  const drawer = volList ? { ...volList, key: 'vol', slideKey: 'sl' + volSlide,
    ...
    rows: volList.rows.map(x => ({ ...x, key: x.id, selected: !!x.selected, dot: x.state === 'degraded' ? 'var(--error)' : x.state === 'public' ? 'var(--warning)' : 'var(--success)', pinned: s.volPin === x.id, bg: s.volPin === x.id ? 'var(--bg-accent)' : 'transparent', toggle: () => set({ volSel: (s.volSel || []).includes(x.id) ? (s.volSel || []).filter(k => k !== x.id) : [...(s.volSel || []), x.id] }),
      ...
      act: volList.kind === 'workloads' ? () => { c.setState({ screen: 's4', compose: { ...prefillCompose(est), bulk: `${x.id} · ${x.tag || 'untagged'} · ${x.ip}`, qty: 1 }, parsedNote: `Isolate ${x.id} (${x.ip}, ${x.tag || 'untagged'}) in ${volCtx.region}: bring it off the public path.` }); syncHash('s4', s.layer, s.tab); }
        : x.action === 'Attach' ? () => { const place = attachPlace(x); const addr = x.address ? ` (${x.address})` : ''; const bulk = x.address ? `${x.id} · ${x.address}` : (place ? `${x.id} · ${place}` : x.id); c.setState({ screen: 's4', compose: { ...prefillCompose(est), bulk, qty: 1 }, parsedNote: place ? `Attach ${x.id}${addr} in ${place}: one circuit onto the fabric.` : `Attach ${x.id}${addr}: one circuit onto the fabric.` }); syncHash('s4', s.layer, s.tab); }
  ...
  return {
    ...
    fabOpen: fabDrill.length > 0, fabClosed: fabDrill.length === 0, sitesDoor, bandDoor, cloudsDoor, fabRows, fabHead, fabUp, fabTrail, hasFabMore: !!(fabHead && fabHead.more), fabMore: fabHead ? fabHead.more : '', openBandLevel: () => openLevel('fabric'), fabHeadY: L.bandY + 8, bandX: L.bandX, bandW: L.bandW, bandLabelX: L.bandX, laneX: L.lane.x, laneW: L.lane.w, strataX: L.bandX + 12, strataW: L.bandW - 24,
...
function costVals(s, set, est, invAll, ob, go, c) {
  ...
  const costDonuts = [
    ...
    R.donut([
      { label: 'On the AT&T fabric', v: fabPart, color: 'var(--viz-1)', sub: `${siteRows.filter(r => r.pubPart === 0).length} of ${siteRows.length} first miles` },
