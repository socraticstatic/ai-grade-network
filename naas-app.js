...
export function vals(c) {
  ...
  const bulkAttach = () => { if (!volList) return; const cnt = volList.bulk.attach; const what = `${cnt.toLocaleString('en-US')} ${volList.rows[0] ? (S.CLASS[vol.cls] || S.CLASS.Branch).plural : 'sites'} in ${S.labelOfKey(est, vol.metro)} on a public first mile`; c.setState({ screen: 's4', compose: { ...prefillCompose(est), bulk: what }, parsedNote: `Attach ${what}. One order, one policy, ${cnt.toLocaleString('en-US')} circuits.` }); syncHash('s4', s.layer, s.tab); };
