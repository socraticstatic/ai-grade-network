...
/* Artifact renderers for the conversation thread. Each artifact appears
 * inline, right after the words that introduced it - no separate pane, no
 * boxed sub-scroll. Everything renders from the engine at paint time. */
...
export function CanvasArtifact({
  ...
  if (item.kind === 'headstart') {
    ...
    return (
      <section data-testid="advisor-headstart" className="rounded-2xl bg-fw-wash p-6">
        ...
        <div className="mt-4 space-y-2">
          {hs.byClass.map(row => (
            <div key={row.siteClass} className="flex items-center gap-3">
              ...
              <div className="h-2 flex-1 overflow-hidden rounded-full bg-fw-base">
                <div
  ...
  if (item.kind === 'scan') {
    return (
      <section data-testid="advisor-scan" className="rounded-2xl bg-fw-wash p-6">
  ...
  if (item.kind === 'finding') {
    return <FindingCard finding={item.finding} onAcceptTier={onAcceptTier} />;
  }
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 max-w-md text-figma-sm text-white/85">
        {head.findings} findings, every figure priced from your own traffic on paths AT&T already runs. Change anything
        and I re-check the number.
      </p>
    </section>
  );
}
