...
All logos, icons, fonts, provider marks, and token files ship in `ATT_NaaS_Brand_Package/` beside this document. Its README carries the usage rules. The short version:

- Logo: the AT&T globe (`logo/att-globe.svg`) never changes color on either theme. The wordmark is two bold words: "AT&T" in AT&T blue `#009fdb`, "AI-grade network" in gray 900 on light and `#f2f6fa` on dark. Wordmark in the top bar at 24px, globe at 56px on the front door and 40px in cards. Never both on one surface. Clear space of one globe height.
- Icons: the curated 26 AT&T icons the apps use in `icons/light/`, `icons/dark/`, and `icons/currentColor/`, plus the full AT&T set of 751 in `icons/all/` in the same three variants with a contact sheet and a searchable data file. 96 grid, outline stroke 4. Default 20px. Always paired with a label. AT&T icons carry meaning (layers, verbs, products); Lucide is only for utility chrome. A concept the 26 do not cover is taken from the full set, never from Lucide.
- Dark surfaces: wash `#111821`, base `#1a2431`, neutral `#222e3c`, hairline `#2f3d4d`, heading `#f2f6fa`, body `#c5cfd9`, link `#66c8f0`. Primary button stays cobalt 600 on both themes and brightens on hover in dark. Wash is never pure black.
- Fonts: install `fonts/All_ATTAleck_Web_Fonts.zip` before opening a canvas.
- Provider marks in `providers/` at 20px inside region nodes, never recolored.
- Never draw a logo, icon, or provider mark by hand. If one is missing from the package, stop and ask.
