...
export function fabricRows(est, inv, ob, trail) {
  ...
  if (trail.length === 1) return { level: 'facility', label: 'AT&T network', rows: facs.map(f => ({ key: f.key, name: f.name, sub: f.sub, state: f.state, drill: f.city, count: `${f.ports}` })), head: plural(facs.length, 'facility', 'facilities'), empty: facs.length === 0, emptyHead: 'Nothing on the fabric yet', emptyLine: 'Attach a cloud region and the AT&T facility carrying it appears here, with its ports and the circuits on them.', emptyCta: 'Attach a region' };
